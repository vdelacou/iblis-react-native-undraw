import React from 'react';
import Svg, { Circle, Defs, G, Path, Rect, Stop, SvgProps } from 'react-native-svg';
import { ClipPath } from '../hack/clip_path';
import { LinearGradient } from '../hack/linear_gradient';
import { IIblisReactNativeUndrawProps } from '../iblis_react_native_undraw_props';
export const SvgImages = (props: IIblisReactNativeUndrawProps & SvgProps) => (
  <Svg data-name="Layer 1" viewBox="0 0 922.73 636.56" style={props.style} width="100%" height={props.height} {...props}>
    <Defs>
      <LinearGradient id="Images_svg__a" x1={704.04} y1={628.14} x2={704.04} y2={138.69} gradientUnits="userSpaceOnUse">
        <Stop offset={0.01} stopColor="gray" stopOpacity={0.25} />
        <Stop offset={0.54} stopColor="gray" stopOpacity={0.12} />
        <Stop offset={1} stopColor="gray" stopOpacity={0.1} />
      </LinearGradient>
      <LinearGradient id="Images_svg__b" x1={560.05} y1={462.46} x2={560.05} y2={29.48} xlinkHref="#Images_svg__a" />
      <LinearGradient id="Images_svg__d" x1={600} y1={694.72} x2={600} y2={205.28} xlinkHref="#Images_svg__a" />
      <LinearGradient id="Images_svg__e" x1={456.01} y1={529.04} x2={456.01} y2={96.07} xlinkHref="#Images_svg__a" />
      <LinearGradient id="Images_svg__g" x1={495.96} y1={761.31} x2={495.96} y2={271.86} xlinkHref="#Images_svg__a" />
      <LinearGradient id="Images_svg__h" x1={351.97} y1={595.63} x2={351.97} y2={162.65} xlinkHref="#Images_svg__a" />
      <ClipPath id="Images_svg__c" transform="translate(-144.57 -138.69)">
        <Rect x={395.46} y={178.13} width={617.65} height={410.82} rx={8.85} ry={8.85} fill="#fff" />
      </ClipPath>
      <ClipPath id="Images_svg__f" transform="translate(-144.57 -138.69)">
        <Rect x={291.42} y={244.72} width={617.65} height={410.82} rx={8.85} ry={8.85} fill="#fff" />
      </ClipPath>
      <ClipPath id="Images_svg__i" transform="translate(-144.57 -138.69)">
        <Rect x={187.38} y={311.3} width={617.65} height={410.82} rx={8.85} ry={8.85} fill="#fff" />
      </ClipPath>
    </Defs>
    <Path
      d="M1055 612.72a15.34 15.34 0 01-15.26 15.42h-671.4a15.34 15.34 0 01-15.26-15.42V154.11a15.34 15.34 0 0115.26-15.42h671.4a15.34 15.34 0 0115.26 15.42"
      transform="translate(-144.57 -138.69)"
      fill="url(#Images_svg__a)"
    />
    <Path d="M902.54 466.52a14.91 14.91 0 01-14.91 14.91H231.8a14.91 14.91 0 01-14.91-14.91V23.19A14.91 14.91 0 01231.8 8.31h655.83a14.91 14.91 0 0114.91 14.91" fill="#f5f5f5" />
    <Rect data-name="&lt;Rectangle&gt;" x={243.09} y={29.48} width={633.92} height={432.98} rx={8.85} ry={8.85} fill="url(#Images_svg__b)" />
    <Rect x={250.89} y={39.44} width={617.65} height={410.82} rx={8.85} ry={8.85} fill="#fff" />
    <G clipPath="url(#Images_svg__c)">
      <Path
        d="M216.54 446.49L388.98 248.9a34.33 34.33 0 0148.57-3.17l48.29 42.58a34.33 34.33 0 0044.8.54l147.58-124.03a34.33 34.33 0 0147.49 3.19l181.11 199.3a34.33 34.33 0 018.78 20l7 78.63a34.33 34.33 0 01-34.17 37.45h-646a34.33 34.33 0 01-34.31-33.29 34.33 34.33 0 018.42-23.61z"
        fill={props.primarycolor}
      />
    </G>
    <Circle cx={325.27} cy={105.32} r={38.25} fill="#ff5252" />
    <Path
      d="M951 679.3a15.34 15.34 0 01-15.26 15.42H264.3A15.34 15.34 0 01249 679.3V220.7a15.34 15.34 0 0115.26-15.42H935.7A15.34 15.34 0 01951 220.7"
      transform="translate(-144.57 -138.69)"
      fill="url(#Images_svg__d)"
    />
    <Path d="M798.5 533.11a14.91 14.91 0 01-14.91 14.91H127.76a14.91 14.91 0 01-14.91-14.91V89.77a14.91 14.91 0 0114.91-14.91h655.83a14.91 14.91 0 0114.91 14.91" fill="#f5f5f5" />
    <Rect data-name="&lt;Rectangle&gt;" x={139.05} y={96.07} width={633.92} height={432.98} rx={8.85} ry={8.85} fill="url(#Images_svg__e)" />
    <Rect x={146.85} y={106.03} width={617.65} height={410.82} rx={8.85} ry={8.85} fill="#fff" />
    <G clipPath="url(#Images_svg__f)">
      <Path
        d="M112.5 513.08l172.44-197.6a34.33 34.33 0 0148.58-3.17l48.28 42.6a34.33 34.33 0 0044.8.54L574.18 231.4a34.33 34.33 0 0147.49 3.19l181.1 199.34a34.33 34.33 0 018.78 20l7 78.63a34.33 34.33 0 01-34.19 37.36h-646a34.33 34.33 0 01-34.31-33.29 34.33 34.33 0 018.45-23.55z"
        fill={props.primarycolor}
      />
    </G>
    <Circle cx={221.23} cy={171.9} r={38.25} fill="#ff5252" />
    <Path
      d="M846.92 745.89a15.34 15.34 0 01-15.26 15.42h-671.4A15.34 15.34 0 01145 745.89V287.28a15.34 15.34 0 0115.26-15.42h671.4a15.34 15.34 0 0115.26 15.42"
      transform="translate(-144.57 -138.69)"
      fill="url(#Images_svg__g)"
    />
    <Path d="M694.43 599.69a14.91 14.91 0 01-14.91 14.91H23.72a14.91 14.91 0 01-14.91-14.91V156.31a14.91 14.91 0 0114.91-14.91h655.83a14.91 14.91 0 0114.88 14.91" fill="#fff" />
    <Rect data-name="&lt;Rectangle&gt;" x={35.01} y={162.65} width={633.92} height={432.98} rx={8.85} ry={8.85} fill="url(#Images_svg__h)" />
    <Rect x={42.81} y={172.61} width={617.65} height={410.82} rx={8.85} ry={8.85} fill="#fff" />
    <G clipPath="url(#Images_svg__i)">
      <Path
        d="M8.43 579.66l172.48-197.59a34.33 34.33 0 0148.52-3.17l48.28 42.6a34.33 34.33 0 0044.8.54l147.63-124.05a34.33 34.33 0 0147.49 3.19l181.1 199.34a34.33 34.33 0 018.78 20l7 78.63a34.33 34.33 0 01-34.19 37.36h-646a34.33 34.33 0 01-34.31-33.2 34.33 34.33 0 018.42-23.65z"
        fill={props.primarycolor}
      />
    </G>
    <Circle cx={117.19} cy={238.49} r={38.25} fill="#ff5252" />
  </Svg>
);
SvgImages.defaultProps = {
  primarycolor: '#6c63ff',
  accentcolor: '#3f3d56',
  haircolor: '#2f2e41',
  skincolor: '#ffb9b9',
  height: 200,
};
